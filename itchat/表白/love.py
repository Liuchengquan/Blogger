...
#通过super初始化父类，__init__()函数无self，若直接QtWidgets.QWidget.__init__(self)，括号里是有self的
        self.setGeometry(300, 300, 500,400)  # setGeometry()方法完成两个功能--设置窗口在屏幕上的位置和设置窗口本身的大小。它的前两个参数是窗口在屏幕上的x和y坐标。后两个参数是窗口本身的宽和高
        ...
        self.setWindowIcon(QIcon('表白.png'))  # 调用QIcon构造函数时，我们需要提供要显示的图标的路径(相对或绝对路径)。同时注意：使用QIcon类型必须导入此模块from PyQt5.QtGui import QIcon
       # self.setToolTip()#调用setToolTip()方法,该方法接受富文本格式的参数,css之类。
        ...
        self.window_pale.setBrush(self.backgroundRole(), QtGui.QBrush(QtGui.QPixmap('表白.png')))
        ...
        # Qt::NoFocus 0 不接受焦点
        self.buttonOK.move(50, 350)  # move()方法来指定部件的放置坐标，坐标的顶点就是窗口的左上角
        ...
        # Qt::WheelFocus StrongFocus | 0x4  滑轮作为焦点选中事件
        # Qt::NoFocus 0 不接受焦点
        self.buttonE.move(200, 350)  # move()方法来指定部件的放置坐标，坐标的顶点就是窗口的左上角
        ...
        # Qt::WheelFocus StrongFocus | 0x4  滑轮作为焦点选中事件
        # Qt::NoFocus 0 不接受焦点
        self.buttonNO.move(350, 350)  # move()方法来指定部件的放置坐标，坐标的顶点就是窗口的左上角
    ...
    def showDialogNO(self):
        self.q = random.randint(0, 400)  # 在生成随机的X坐标
        self.w = random.randint(100, 300)  # 在150-650内生成随机的Y坐标
